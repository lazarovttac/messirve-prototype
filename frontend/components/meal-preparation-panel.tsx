  ...
  return (
    <Card className="shadow-md rounded-2xl border-none">
      <CardHeader>
        ...
        <CardDescription>
          {sortedMeals.filter((m) => m.status !== "completed").length} comidas
          para preparar
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="meals" onValueChange={setView}>
          ...
          <TabsContent value="meals" className="mt-0">
            <div className="space-y-4">
              {sortedMeals.filter((meal) => meal.status !== "completed")
                .length === 0 ? (
                <div className="text-center py-4 text-muted-foreground">
                  No hay comidas para preparar
                </div>
              ) : (
                sortedMeals
          ...
          <TabsContent value="tables" className="mt-0">
            <div className="space-y-4">
              {tableData.length === 0 ? (
                <div className="text-center py-4 text-muted-foreground">
                  No mesas con comidas para preparar
                </div>
              ) : (
                tableData.map((tableInfo: any, index) => (
                  <div
                    ...
                    className="border rounded-xl p-3 space-y-2 shadow-sm"
                  >
                    <div className="font-medium">Mesa {tableInfo.table}</div>
